...
void ServerWindow::dataReceived()
{
    ...
    if (user == nullptr) cout << "getUserBySocket returned null user" << endl;
    ...
    QVariant serialized = QVariant::fromValue(user);
    //QVariant serialized = QVariant::fromValue(test);
    in >> serialized;
    //serialized.setValue(user);
...
void ServerWindow::sendAll(const QString &message)
{
    ...
    {
        cout << "message sent to one user in loop : " << message.toStdString() << endl;
        QTcpSocket* socket = clients[i]->getSocket();
        cout << "plop" << endl;
        socket->write(paquet);
    }
}
